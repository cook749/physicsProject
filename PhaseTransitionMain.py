    ...
    while stop_cond == False and carlo_sweeps < 100000:
        ...
        #Pseudocode yes Ny-1 but remember python is exclusive on upper bound
            ...
            for j in range (0,Nx):
                ...
                if energy_change <= 0:
                    grid[i,j] = flipped # Always accept moves that lower energy
                else:
                    # Only calculate exp for "uphill" moves
                    if np.random.rand() < np.exp(-energy_change/kT):
                        grid[i,j] = flipped
...
def data_sweeps(L,J,B,kT,num_runs):
...
def main():
    exit = False
    while exit == False:
        choice = input("Enter 1 for new equilibrium grid generation, 2 for nucleation data collection, 3 for data analysis, or 'q' to quit: ")
        if choice == '1':
            Nx = int(input("Enter number of grid points in x-direction (Nx): "))
            Ny = int(input("Enter number of grid points in y-direction (Ny): "))
            grid = generate_random_grid(Nx, Ny)
