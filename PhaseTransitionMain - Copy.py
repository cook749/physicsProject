...
def grid_sweep(grid, Nx,Ny,J,B, no_sweeps,kT,snapshot_interval = 1):
    avg_mag_arr = []
    running_avg = []
    snapshots = []
    ...
    return avg_mag_arr , running_avg, snapshots
...
avg_mag_arr, running_average, snapshots = grid_sweep(grid, Nx, Ny, J, B, no_sweeps, kT, snapshot_interval=1)
steady_state_sweep = steady_state(running_average)
animation = input("Do you want to see an animation of the lattice evolution? (y/n): ")
if animation.lower() == 'y':

    animate_snapshots(snapshots, show_animation=True, save_gif=True, save_frames=True, gif_name="ising_evolution_Milestone_100200.gif", fps=7)
